...
var NostrTools = (() => {
  ...
  var hexes = /* @__PURE__ */ Array.from({length: 256}, (_, i2) =>
    i2.toString(16).padStart(2, '0')
  )
  function bytesToHex(bytes4) {
    if (!u8a2(bytes4)) throw new Error('Uint8Array expected')
    let hex2 = ''
  ...
  function invert(number4, modulo) {
    if (number4 === _0n2 || modulo <= _0n2) {
      throw new Error(
        `invert: expected positive integers, got n=${number4} mod=${modulo}`
      )
    }
  ...
  function Field(ORDER, bitLen2, isLE4 = false, redef = {}) {
    ...
    const f = Object.freeze({
      ...
      cmov: (a, b, c) => (c ? b : a),
      toBytes: num =>
        isLE4 ? numberToBytesLE(num, BYTES) : numberToBytesBE(num, BYTES),
      fromBytes: bytes4 => {
        if (bytes4.length !== BYTES)
          throw new Error(
            `Fp.fromBytes: expected ${BYTES}, got ${bytes4.length}`
          )
  ...
  var hexes2 = Array.from({length: 256}, (v, i2) =>
    i2.toString(16).padStart(2, '0')
  )
  function bytesToHex2(bytes4) {
    if (!u8a3(bytes4)) throw new Error('Uint8Array expected')
    let hex2 = ''
    for (let i2 = 0; i2 < bytes4.length; i2++) {
  ...
  var SHA2562 = class extends SHA22 {
    ...
    process(view, offset) {
      ...
      for (let i2 = 0; i2 < 64; i2++) {
        ...
        const T1 =
          (H + sigma1 + Chi2(E, F, G) + SHA256_K2[i2] + SHA256_W2[i2]) | 0
  ...
  function getFilterLimit(filter) {
    ...
    return Math.min(
      ...
      filter.authors?.length &&
        ...
        : Infinity,
      filter.authors?.length &&
        filter.kinds?.every(kind => isAddressableKind(kind)) &&
        filter['#d']?.length
  ...
  var AbstractRelay = class {
    ...
    async waitForPingPong() {
      return new Promise((res, err) => {
        ;(this.ws && this.ws.on && this.ws.on('pong', () => res(true))) ||
          err("ws can't listen for pong")
    ...
    async pingpong() {
      if (this.ws?.readyState === 1) {
        const result = await Promise.any([
          this.ws && this.ws.ping && this.ws.on
            ? this.waitForPingPong()
            : this.waitForDummyReq(),
  ...
  var AbstractSimplePool = class {
    ...
    subscribeMap(requests, params) {
      ...
      const groupedRequests = Array.from(grouped.entries()).map(
              ...
              if (reason.startsWith('auth-required: ') && params.onauth) {
                relay
                  ...
                  .catch(err => {
                    handleClose(
                      i2,
                      `auth was required and attempted, but failed with: ${err}`
                    )
                  })
              } else {
      ...
      encode: digits => {
        ...
        return digits.map(i2 => {
          ...
          if (i2 < 0 || i2 >= alphabet2.length)
            throw new Error(
              `Digit index outside alphabet: ${i2} (alphabet: ${alphabet2.length})`
            )
          return alphabet2[i2]
        })
      },
  ...
  var sbox = /* @__PURE__ */ (() => {
    ...
    for (let i2 = 0, x = 1; i2 < 256; i2++, x ^= mul22(x)) t[i2] = x
  ...
  var xPowers = /* @__PURE__ */ (() => {
    ...
    for (let i2 = 0, x = 1; i2 < 16; i2++, x = mul22(x)) p[i2] = x
    return p
  })()
  function expandKeyLE(key) {
    ...
    for (let i2 = Nk; i2 < xk.length; i2++) {
      let t = xk[i2 - 1]
      if (i2 % Nk === 0) t = subByte(rotr32_8(t)) ^ xPowers[i2 / Nk - 1]
      else if (Nk > 6 && i2 % Nk === 4) t = subByte(t)
      xk[i2] = xk[i2 - Nk] ^ t
    }
    return xk
  }
  function expandKeyDecLE(key) {
    ...
    for (let i2 = 0; i2 < Nk; i2 += 4) {
      for (let j = 0; j < 4; j++) xk[i2 + j] = encKey[Nk - i2 - 4 + j]
    }
    encKey.fill(0)
  ...
  var gcm = wrapCipher(
    ...
    function gcm2(key, nonce, AAD) {
      ...
      function _computeTag(authKey, tagMask, data) {
        ...
        for (let i2 = 0; i2 < tagMask.length; i2++) tag[i2] ^= tagMask[i2]
        return tag
      }
      function deriveKeys() {
        const xk = expandKeyLE(key)
  ...
  var siv = wrapCipher(
    ...
    function siv2(key, nonce, AAD) {
      ...
      function _computeTag(encKey, authKey, data) {
        ...
        for (let i2 = 0; i2 < 12; i2++) tag[i2] ^= nonce[i2]
        tag[15] &= 127
        const t32 = u32(tag)
        let s0 = t32[0],
          s1 = t32[1],
  ...
  var Poly1305 = class {
    constructor(key) {
      ...
      this.r[9] = (t7 >>> 5) & 127
      for (let i2 = 0; i2 < 8; i2++) this.pad[i2] = u8to16(key, 16 + 2 * i2)
    }
    ...
    finalize() {
      ...
      g[9] -= 1 << 13
      let mask = (c ^ 1) - 1
      for (let i2 = 0; i2 < 10; i2++) g[i2] &= mask
      mask = ~mask
      for (let i2 = 0; i2 < 10; i2++) h[i2] = (h[i2] & mask) | g[i2]
  ...
  function extract(hash3, ikm, salt) {
    assert_default.hash(hash3)
    if (salt === void 0) salt = new Uint8Array(hash3.outputLen)
  ...
  var nip44Encrypt = (data, privateKey, publicKey) =>
    encrypt3(JSON.stringify(data), nip44ConversationKey(privateKey, publicKey))
  var nip44Decrypt = (data, privateKey) =>
    JSON.parse(
      decrypt3(data.content, nip44ConversationKey(privateKey, data.pubkey))
    )
  ...
  async function makeNwcRequestEvent(pubkey, secretKey, invoice) {
    ...
    const encryptedContent = encrypt2(
      secretKey,
      pubkey,
